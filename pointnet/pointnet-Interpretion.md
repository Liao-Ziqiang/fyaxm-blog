...
- ②③④句作者用几句话论述了为什么这样一个排序算法是无法找到的，但作者说得比较模糊。下面我尝试用更加精确的语言进行分析。由于本人水平原因，下面的数学论述可能有错误，而且并没有直接证明该结论，但我希望提供一个分析的思路供读者理解。
...
- 上面我们对问题进行了更加严谨、清晰地定义。不过由于本人水平原因，无法继续给出严谨的证明，当然论文作者也没有。不过为了方便读者理解、感受为什么找到这样一个排序算法是困难的，我们可以继续做如下思考：
...
- 这里展示了PointNet的通用性，它不仅接受维度为3的点云，我们可以为它加入RGB值和其他信息。
...
- 这里需要解释什么是 最远点采样（Furthest Sampling）和 随机采样（Random Sampling）。
  - 最远采样是一种基于距离的采样策略，其目标是从数据集中选择尽可能分散的数据点。采样方法是，选择一个点，加入到已选集合中，然后找到距离已选择的点最远的一个点加入到集合，如此循环。这种方法能够确保所选数据点具有最大的空间覆盖范围，可以更好地代表整个数据分布。
  - 随机采样则不考虑空间分布，每个数据点被选择的概率是相同的。
